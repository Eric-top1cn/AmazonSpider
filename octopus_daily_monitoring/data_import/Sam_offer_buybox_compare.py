...
def sam_buybox_data_process(file):
    if file.split('.')[-1] == 'xlsx': file = file.replace('.xlsx','.xls')

    td = get_current_date()
    yt = get_yesterday_date()
    if mongo_config(f'Sam_Buybox_{td}').find().count() == 0 or mongo_config(f'Sam_Offer_{td}').find().count()==0:
        print('Sam buybox 爬虫未运行完')
        return 1
