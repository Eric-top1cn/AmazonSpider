...
def main(country,cycle_time=10):
    country = country.lower()
    asin_list = pd.DataFrame(pd.read_excel(f'{country.upper()}_asin.xlsx'))['ASIN'].tolist()
    post_code_file = f'{country.upper()}_post_code.txt'
    postal_code_list = get_post_code_list(post_code_file)
    ...
    for postal_code in postal_code_list:
        for num in range(cycle_time):#设置每个邮编要生成的cookie个数
            ...
            url = f'http://www.amazon.{url_suffix.get(country)}/dp/{asin}'#生成对应国家的产品listing页面
            cookies = get_driver_cookie(url,postal_code)#获取cookies
            save_cookies(cookies,f'{country.upper()}_cookies_{date}.log')#保存cookies


def start_cookie_pool_update(country,cycle_time=10):
...
if __name__ == '__main__':
    ...
    # for country in ['uk','us','de']:
    #     main(country)
    # driver = switch_to_browser()
    # driver.get('http://www.baidu.com')
    # file = r'C:\Users\Administrator\pycharm_code\Sam\uk_de_listing_offer'
    # date = get_current_date()
    # cookie_save_path = r'C:\Users\Administrator\pycharm_code\Sam\uk_de_listing_offer'
    # country_list = ['uk', 'de']
    # de_asin_list = pd.DataFrame(pd.read_excel(f'{file}\\DE_asin.xlsx'))['ASIN'].tolist()
    # post_code = random.choice(get_post_code_list(f'{file}\\DE_post_code.txt'))
    # for i in range(10):
    #     driver = switch_to_browser(img_load=1, headless=False)
    #     driver.set_page_load_timeout(10)
    #     asin = random.choice(de_asin_list)
    #     driver.get(f'http://www.amazon.de/dp/{asin}')
    #     change_postal_code(driver, post_code)
    #     driver.refresh()
    #     cookies = driver.get_cookies()
    #     # save_cookies(cookies, f'{cookie_save_path}\\DE_cookies_{date}.txt')
    #     driver.quit()